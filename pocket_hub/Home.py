...
try:
    ...
    with col_alerts:
        ...
        # 0. Database Status
        try:
            # Force absolute path calculation for cloud stability
            core_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../pocket_core'))
            ...
            st.error(f"❌ Failed to import 'db'. Path: {sys.path}")
            alerts_found = True
